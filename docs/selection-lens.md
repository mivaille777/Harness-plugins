...
The Lens reads `selection.current` through the native bridge. After Harness accepts a browser selection, Native emits a content-free `selection-captured` signal; while the Lens is idle, it rereads and displays the canonical snapshot immediately. A compact source strip identifies the application and document, followed by the selected text, so the user can verify the captured material before acting. While an Explain or Ask request is active, the Lens defers a newer selection until that request reaches a terminal state, preserving the fixed material that was submitted. Captured local, section and page text is behind a disclosure panel; a user chooses a supported scope and presses **Load context** to request a bounded preview tied to the same snapshot revision. The preview is reference-only and is never added to a request automatically.
