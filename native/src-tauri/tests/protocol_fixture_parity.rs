...
fn every_shared_golden_fixture_matches_the_rust_v4_contract() {
    ...
    assert!(
        !files.is_empty(),
        "shared protocol fixture set must not be empty"
    );

    for path in files {
        let source = read_to_string(&path)
            .unwrap_or_else(|error| panic!("cannot read fixture {}: {error}", path.display()));
        IpcMessage::from_json(&source).unwrap_or_else(|error| {
            panic!(
                "fixture {} violated the Rust contract: {error}",
                path.display()
            )
        });
    }
...
fn every_shared_invalid_fixture_is_rejected_by_the_rust_v4_contract() {
    ...
    assert!(
        !files.is_empty(),
        "shared invalid protocol fixture set must not be empty"
    );

    for path in files {
        let source = read_to_string(&path).unwrap_or_else(|error| {
            panic!("cannot read invalid fixture {}: {error}", path.display())
        });
        assert!(
