...
describe('selection lens', () => {
  ...
  it('renders correlated text and completes only on turn end', async () => {
    ...
    eventApi.handler?.({ payload: { sessionId: 'session-1', requestId: 'request-1', event: { kind: 'assistant-delta', data: { cursor: 3, value: { turn: 1, step: 1, value: { type: 'text-delta', index: 0, text: 'Actual answer' } } } } } })
    ...
    eventApi.handler?.({ payload: { sessionId: 'session-1', requestId: 'request-1', event: { kind: 'assistant-complete', data: { cursor: 4, value: { turn: 1, step: 1, value: { content: [{ type: 'text', text: 'Actual answer' }] } } } } } })
    expect(screen.getByText('Receiving answer')).toBeInTheDocument()
    eventApi.handler?.({ payload: { sessionId: 'session-1', requestId: 'request-1', event: { kind: 'status', data: { cursor: 5, value: { status: 'turn-end', turn: 1, reason: { kind: 'completed' } } } } } })
  ...
  it('requests cancellation and waits for the correlated aborted turn end', async () => {
    ...
    eventApi.handler?.({ payload: { sessionId: 'session-1', requestId: 'request-1', event: { kind: 'status', data: { cursor: 6, value: { status: 'turn-end', turn: 1, reason: { kind: 'aborted', cause: { kind: 'user' } } } } } } })
